...
inline cv::Mat getTransformMatrixSafas(int x, int y, point_t* pts) {
    cv::Mat dst_pts = (cv::Mat_<float>(5,2) << 30.2946, 51.6963, 
                    65.5318, 51.5014, 
                    48.0252, 71.7366, 
                    33.5493, 92.3655, 
                    62.7299, 92.2041);
    
    cv::Mat src_pts = (cv::Mat_<float>(5,2) << pts[0].x - x, pts[0].y - y,
            pts[1].x - x, pts[1].y - y,
            pts[2].x - x, pts[2].y - y,
            pts[3].x - x, pts[3].y - y,
            pts[4].x - x, pts[4].y - y);
    ...
    cv::Mat src_mat(5, 2, CV_32F);
    src_mat.data = (uchar *)src_pts.data;
    cv::Mat dst_mat(5, 2, CV_32F);
    dst_mat.data = (uchar *) dst_pts.data;
    
    cv::Mat M_temp = similarTransform(src_mat, dst_mat);
    cv::Mat M = M_temp.rowRange(0, 2);

    return M;
}

inline void toTransform(const cv::Mat& img, cv::Mat &transformed, cv::Mat& transM, cv::Size size) {
